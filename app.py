...
#createする前に、ターミナルで「py -3」で対話モードにして、「from app import db」をするのか？
#下のcreateは、ターミナルの対話モードで行うのが慣例。
...
def signup():
    ...
    if request.method =="POST":
        ...
        #上で作成したuserカラムごとに入力された値を放り込むよ。なお、パスはハッシュ化しますよ。
        ...
        #elseは、get（サイトを閲覧するだけ。）の場合は、サインイン画面見ることができる。  
...
def login():
    ...
    if request.method =="POST":
        ...
        #ログインなので審査しないといけなくて、DBを見るのではなく、クエリを見て、入力値と比べて審査する。
        #そして何個も同じ名前があっても、最初の該当の名前を取る。
        user = User.query.filter_by(username=username).first()
        #取ってきた値はハッシュで秘匿化されている状態だけど審査してログイン
...
def create():
    ...
    if request.method =="POST":
        ...
        #elseは、get（サイトを閲覧するだけ。）の場合は、入力フォーム見ることができる。
